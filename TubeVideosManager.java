...
public class TubeVideosManager implements TubeVideosManagerInt {
	...
	public boolean addVideoToDB(String title, String url, int durationInMinutes, Genre videoGenre) {
		//throw new UnsupportedOperationException("Implement this method");
		videoDatabase.add(new Video(title,url,durationInMinutes,videoGenre));
		return true;
	}
	...
	public ArrayList<Video> getAllVideosInDB() {
		//throw new UnsupportedOperationException("Implement this method");
		ArrayList<Video> newDb = new ArrayList<Video>();
		for(Video v : videoDatabase) {
			newDb.add(new Video(v.getTitle(),v.getUrl(),v.getDurationInMinutes(),v.getGenre()));
		}
		return newDb;
	}
	...
	public Video findVideo(String title) {
		...
		for(Video v:videoDatabase) {
			if(v.getTitle().equals(title)){
				foundVideo=new Video(v.getTitle(),v.getUrl(),v.getDurationInMinutes(),v.getGenre());
			}
		}
		return foundVideo;
	}
	...
	public boolean addVideoToPlaylist(String title, String playlistName) {
		//Establishes check booleans for the two requirements
		boolean check1=false;
		boolean check2=false;
		//Scans the database for a Video that has the matching title
		for(Video v : videoDatabase) {
			if(v.getTitle().equals(title)) {
				check1=true;
			}
		}
		//Checks for a Playlist that has the matching name
		Playlist possiblePlaylist=null;
		for(Playlist p : playlists){
		...
		/*
		 * If the Video exists and the Playlist does too, the video name is added to the
		 * chosen Playlist as it's not making a new Playlist and references the existing object
		 */
		if(check1&&check2) {
			possiblePlaylist.addToPlaylist(title);
			return true;
		}
		return false;
	}
	...
	public Playlist getPlaylist(String playlistName) {
		//Determines if parameter is valid
		if(playlistName.isBlank()) {
			throw new IllegalArgumentException("Invalid Parameters");
		}
		//Scans all Playlists in the playlists ArrayList
		for(Playlist p:playlists) {
			//Checks to see if there exists a Playlist with the given name
			if(p.getName().equals(playlistName)){
				/*
				 * Returns a new Playlist using the second playlist constructor that takes
				 * an existing Playlist as a parameter
				 */
	...
	public Playlist searchForVideos(String playlistName, String title, int maximumDurationInMinutes, Genre genre) {
		//Search database to make a new Playlist that contains all videos matching relevant parameters
		Playlist p = new Playlist(playlistName);
		for(Video v:videoDatabase){
			if(title!=null&&!title.equals(v.getTitle())) {
				continue;
			}
			if(maximumDurationInMinutes!=0&&maximumDurationInMinutes!=v.getDurationInMinutes()){
				continue;
			}
			if(!genre.equals(null)&&!genre.equals(v.getGenre())) {
				continue;
			}
			p.addToPlaylist(title);
		}
		return p;
	}
	...
	private Playlist findPlayListInternal(String playlistName) {
		...
		//throw new UnsupportedOperationException("Complete this method after removing this line");
		for(Playlist p:playlists) {
			if(p.getName().equals(playlistName)) {
				return p;
			}
